  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      if (isCacheValid(cached)) {
        console.log(`✅ Pricing cache HIT for ${checkIn} to ${checkOut} (from file)`);
        return cached.data;
      } else {
        console.log(`⏰ Pricing cache EXPIRED for ${checkIn} to ${checkOut}`);
        fs.unlinkSync(filePath); // Delete expired cache
      }
    }
  } catch (error) {
    console.error(`Error reading pricing cache for ${checkIn} to ${checkOut}:`, error);
  }
...
export function setCachedPricing(checkIn, checkOut, guests, data) {
  ...
  try {
    ...
    console.log(`💾 Cached pricing for ${checkIn} to ${checkOut} to file`);
  } catch (error) {
    console.error(`Error writing pricing cache for ${checkIn} to ${checkOut}:`, error);
  }
}
