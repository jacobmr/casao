...
export async function createFamilyBooking(booking) {
  try {
    ...
    await redis.set(`family:bookings:${id}`, JSON.stringify(familyBooking));
...
export async function updateFamilyBooking(id, updates) {
  try {
    ...
    await redis.set(`family:bookings:${id}`, JSON.stringify(updated));
...
export async function createFamilySession() {
  try {
    ...
    console.log(
      `✅ Created family session ${sessionToken} (expires in 30 days)`,
    );
    return sessionToken;
  } catch (error) {
