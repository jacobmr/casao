 ...
 */
export async function getFamilyBookings(
  from: string,
  to: string,
  ...
  try {
    ...
    return events.map((event) => {
      ...
      const isPending =
        title.toLowerCase().startsWith("pending:") ||
        title.toLowerCase().startsWith("pending ");
  ...
  try {
    const response = await calendar.events.insert({
      ...
      requestBody: {
        summary: title,
        description:
          data.notes || `Family booking request for ${data.guestName}`,
        start: {
...
export async function getConfirmedBookings(
  ...
  return allBookings.filter((b) => {
    ...
    // Exclude [KINDRED] events (handled separately)
    if (b.title.startsWith("[KINDRED]")) return false;
    ...
    if (b.title === b.title.toUpperCase() && !b.title.includes(" "))
      return false;
...
/**
 * Kindred booking (home exchange via livekindred.com)
 * Synced from calendar invites via kindred-calendar-sync.gs
 */
export interface KindredBooking {
  checkIn: string; // YYYY-MM-DD
  checkOut: string; // YYYY-MM-DD
  guestName: string;
}
...
/**
 * Get Kindred bookings from Google Calendar (events with [KINDRED] prefix)
 * These are synced from Kindred calendar invites via kindred-calendar-sync.gs
 * @param from - Start date (YYYY-MM-DD)
 * @param to - End date (YYYY-MM-DD)
 */
export async function getKindredBookings(
  from: string,
  to: string,
): Promise<KindredBooking[]> {
  const calendar = getCalendarClient();
  const calendarId = process.env.GOOGLE_CALENDAR_ID;
  ...
  try {
    ...
    // Filter for [KINDRED] prefixed events and extract guest name
    return events
      .filter((event) => event.summary?.startsWith("[KINDRED]"))
      .map((event) => ({
        checkIn:
          event.start?.date || event.start?.dateTime?.split("T")[0] || "",
        checkOut: event.end?.date || event.end?.dateTime?.split("T")[0] || "",
        guestName: event.summary!.replace("[KINDRED]", "").trim(),
      }));
  } catch (error) {
    console.error("Error fetching Kindred bookings:", error);
    return [];
  }
}
