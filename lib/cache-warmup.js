  ...
  warmupPromise = (async () => {
    try {
      ...
      for (let i = 0; i < MONTHS_TO_CACHE; i++) {
        ...
        console.log(
          `  📅 Warming up ${targetDate.toLocaleDateString("en-US", { month: "long", year: "numeric" })}...`,
        );

        try {
          // Fetch availability (will cache it)
          const availResponse = await fetch(
            `http://localhost:${process.env.PORT || 3000}/api/calendar?from=${from}&to=${to}`,
          );
