...
async function fetchMonthlyPricing(availabilityData) {
  ...
  for (let i = 1; i < availableDates.length; i++) {
    ...
    if (daysDiff <= 1 && currentChunk.length < 7) {
      ...
      // Start new chunk
      if (currentChunk.length >= 1) { // Fetch pricing for any chunk (even single days)
  ...
  // Add last chunk
  if (currentChunk.length >= 1) { // Include all remaining dates
    weeklyChunks.push(currentChunk);
  }
  ...
  for (const chunk of weeklyChunks) {
    ...
    if (nights < 3) {
      ...
      console.log(`  ⚠️  Chunk too short (${nights} nights), extending to 3 nights: ${checkInStr} to ${checkOutStr}`);
    }
