...
import { getCachedToken } from "./token-service-kv.js";
...
async function fetchMonthlyPricing(availabilityData) {
  ...
  console.log(
    `💰 Fetching pricing for ${availableDates.length} available dates...`,
  );
  ...
  for (let i = 1; i < availableDates.length; i++) {
    ...
    if (daysDiff <= 1 && currentChunk.length < 7) {
      ...
      if (currentChunk.length >= 1) {
        // Fetch pricing for any chunk (even single days)
  ...
  for (const chunk of weeklyChunks) {
    ...
    // If less than 3 nights, extend backwards to make it 3 nights (minimum stay requirement)
    if (nights < 3) {
      const adjustedCheckIn = new Date(checkInDate);
      adjustedCheckIn.setDate(adjustedCheckIn.getDate() - (3 - nights));
      checkInStr = adjustedCheckIn.toISOString().split("T")[0];
      console.log(
        `  ⚠️  Chunk too short (${nights} nights), extending to 3 nights: ${checkInStr} to ${checkOutStr}`,
      );
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.warn(
          `  ⚠️  Quote failed for ${checkInStr} to ${checkOutStr} (may have restrictions)`,
        );
      }
      ...
      console.error(
        `  ❌ Error fetching pricing for ${checkInStr}:`,
        error.message,
      );
    }
  }
...
export { fetchMonthlyPricing, getMonthlyPricingData };
