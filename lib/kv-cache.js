...
export async function setCachedAvailability(year, month, data) {
  try {
    ...
    await redis.set(key, JSON.stringify(data), {
      EX: AVAILABILITY_CACHE_DURATION,
    });
    console.log(
      `💾 Cached availability for ${year}-${month} in Redis (15 min TTL)`,
    );
  } catch (error) {
...
export async function getCachedToken() {
  try {
    ...
    if (expiresIn > 60) {
      console.log(
        `✅ Using Redis cached token (valid for ${Math.round(expiresIn / 3600)} hours)`,
      );
      return data.access_token;
    }

    console.log(
      `⚠️  Token in cache but expired or expiring soon (${expiresIn}s left)`,
    );
    return null;
  } catch (error) {
...
export async function setCachedToken(token, expiresAt) {
  try {
    ...
    console.log(
      `✅ Token cached in Redis until ${new Date(expiresAt * 1000).toISOString()}`,
    );
  } catch (error) {
...
export async function getAvailabilityWithFallback(year, month, token) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        `Guesty API error for ${year}-${month}: ${response.status}`,
      );
      return null;
    }
    ...
    console.log(
      `📅 Read-through cache: fetched and cached ${year}-${month} from Guesty`,
    );
