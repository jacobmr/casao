  ...
  try {
    ...
    if (!guestName || !guestEmail || !guestCount || !checkIn || !checkOut) {
      return NextResponse.json(
        {
          error:
            "Missing required fields: guestName, guestEmail, guestCount, checkIn, checkOut",
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          ...
          message:
            "High-season dates should be booked through the main site with a discount code",
          highSeasonDays,
          offSeasonDays,
        },
        { status: 400 },
    ...
    const nightlyRate = useFamilyRate
      ? FAMILY_RATE.nightlyRate
      : OFF_SEASON_DEFAULTS.nightlyRate;
