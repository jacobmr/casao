...
interface CalendarDay {
  date: string;
  status: "available" | "family" | "owner" | "booked" | "kindred";
...
export async function GET(request: Request) {
  try {
    ...
    const guestyGuestsByDate = new Map<
      string,
      {
        guestName: string;
        isCheckIn: boolean;
    ...
    try {
      ...
      console.error(
        "Failed to fetch guest bookings from Google Calendar:",
        error,
      );
    }
    ...
    const familyBookingsByDate = new Map<
      string,
      {
        booking: (typeof familyBookings)[0];
        isCheckIn: boolean;
        isCheckOut: boolean;
      }
    >();
    ...
    familyBookings.forEach((booking) => {
      ...
      while (current < checkOut) {
        ...
        familyBookingsByDate.set(dateStr, {
          booking,
          isCheckIn: isCheckInDate,
          isCheckOut: false,
        });
      ...
      if (!existing) {
        ...
        familyBookingsByDate.set(checkOutDateStr, {
          booking,
          isCheckIn: false,
          isCheckOut: true,
        });
    ...
    // Fetch Kindred bookings from Google Calendar (events with [KINDRED] prefix)
    // These are home exchange guests via livekindred.com
    const kindredBookingsByDate = new Map<
      string,
      {
        guestName: string;
        isCheckIn: boolean;
    ...
    >();
    try {
      const kindredBookings = await getKindredBookings(fromStr, toStr);
      kindredBookings.forEach((booking) => {
        const checkIn = new Date(booking.checkIn);
        const checkOut = new Date(booking.checkOut);
        const current = new Date(checkIn);
        ...
        while (current < checkOut) {
          const dateStr = current.toISOString().split("T")[0];
          const isCheckInDate = dateStr === booking.checkIn;
          kindredBookingsByDate.set(dateStr, {
            guestName: booking.guestName,
            isCheckIn: isCheckInDate,
            isCheckOut: false,
            checkIn: booking.checkIn,
        ...
        // Mark check-out day
        const checkOutDateStr = booking.checkOut;
        if (!kindredBookingsByDate.has(checkOutDateStr)) {
          kindredBookingsByDate.set(checkOutDateStr, {
            guestName: booking.guestName,
            isCheckIn: false,
            isCheckOut: true,
            checkIn: booking.checkIn,
      ...
      console.error(
        "Failed to fetch Kindred bookings from Google Calendar:",
        error,
      );
    }
    ...
    const guestyDataByMonth = new Map<
      string,
    ...
    while (current <= to) {
      ...
      // Check if this date has a Kindred booking
      const kindredData = kindredBookingsByDate.get(dateStr);
      ...
      if (familyData) {
        ...
        // Kindred home exchange booking (typically during owner blocks)
        calendarDays.push({
          date: dateStr,
          status: "kindred",
          isCheckIn: kindredData.isCheckIn,
          isCheckOut: kindredData.isCheckOut,
          booking: {
            title: "Kindred Guest",
            guestName: kindredData.guestName,
            checkIn: kindredData.checkIn,
            checkOut: kindredData.checkOut,
          },
        });
      } else {
    ...
    return NextResponse.json({
      ...
      kindredBookings: kindredBookingsByDate.size > 0 ? 1 : 0, // Count unique stays, not days
    });
  } catch (error) {
